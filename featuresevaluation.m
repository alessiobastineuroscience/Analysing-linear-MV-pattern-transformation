...
for nsubj=1:size(x,2)
    for mruns=1:2
        X=x{mruns,nsubj};
        Y=y{mruns,nsubj}; % in case of real data it is better to use an across-sessions approach
        % computation of optimal parameter, of the estimated transformation
        % and GOF
        [estT,optlambda(mruns),gof(mruns)]=tikregmethod(X,Y,lambdas);
        ...
        [rdsv(mruns)]=deformationfeatures(estT,X,Y);
