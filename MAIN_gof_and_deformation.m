...
resultsMCdeformation=MCproceduredeformation(x,dimx,dimy,dimt,lambdas,deformations,numberofrep);
...
for idef=1:numel(deformations)
   hold on
   errorbar(mean(squeeze(resultsMCdeformation.gof(idef,:,:)),2),mean(squeeze(resultsMCdeformation.rdsv(idef,:,:)),2),std(squeeze(resultsMCdeformation.rdsv(idef,:,:)),0,2),std(squeeze(resultsMCdeformation.rdsv(idef,:,:)),0,2),std(squeeze(resultsMCdeformation.gof(idef,:,:)),0,2),std(squeeze(resultsMCdeformation.gof(idef,:,:)),0,2),'-sb','MarkerSize',6,'MarkerEdgeColor',color{idef},'MarkerFaceColor',color{idef},'DisplayName',strcat('Rate of decay SV=',num2str(deformations(idef))))
end
hold on
errorbar(mean(results.gof),mean(results.rdsv),std(results.rdsv),std(results.rdsv),std(results.gof),std(results.gof),'-sk','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor','k','DisplayName','Estimate on data of interest')
